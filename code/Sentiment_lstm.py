...
def word2vec_train(combined):
    ...
    model.save('../lstm1_data/Word2vec_model.pkl')
    #print(model.wv['物理学'])
    ...
    output = open('../lstm1_data/wordvec.pkl','wb')
...
def get_data(index_dict,word_vectors,combined,y):
    ...
    x_train, x_test, y_train, y_test = train_test_split(combined, y, test_size=0.2)
     ...
     # 读取大语料文本
    f = open('../lstm1_data/wordvec.pkl', 'rb')  # 预先训练好的
...
def train_lstm(n_symbols,embedding_weights,x_train,y_train,x_test,y_test):
    ...
    with open('../lstm1_data/lstm.yml', 'w') as outfile:
        outfile.write( yaml.dump(yaml_string, default_flow_style=True) )
    model.save_weights('../lstm1_data/lstm.h5')
    print('Test score:', score)
...
def get_word2vec():
    f = open('api/lstm4_data/wordvec.pkl', 'rb')  # 预先训练好的
    index_dict = pickle.load(f)  # 索引字典，{单词: 索引数字}
    word_vectors = pickle.load(f)  # 词向量, {单词: 词向量(100维长的数组)}
...
def sentencs():
    try:
        datas = pd.read_excel('E:\我的项目\git项目\opinionanalysis\敏感，非敏感\my_1敏感.xlsx', header=None)
        for i in datas[0]:
            try:
                yield i
            except:
        ...
        with open('E:\我的项目\git项目\opinionanalysis\敏感，非敏感\my_1敏感.xlsx', 'r') as f:
            reader = csv.reader(f)
            for i in reader:
                try:
                    yield i[0]
